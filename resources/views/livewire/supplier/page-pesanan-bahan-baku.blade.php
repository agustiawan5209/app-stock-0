<div>
        ...
        <tbody>
            <x-tr>
               @foreach ($barang as $barangmasuks)
                 ...
                 <x-td>
                    <button type="button" class="btn btn-accent" wire:click='statusPesanan({{$barangmasuks->pesanan->id}})'>
    ...
    <x-jet-dialog-modal wire:model="itemStatus">
        ...
        <x-slot name="footer">
            <button type="button" class="btn btn-nebg-neutral" wire:click="updateStatus({{$itemID}})">Simpan</button>
        </x-slot>
    </x-jet-dialog-modal>
</div>
